...
public static String md5Hash(Object source, Object salt, int iter) {
	Md5Hash md5Hash = new Md5Hash(source, ByteSource.Util.bytes(salt), iter);
	return md5Hash.toHex();
}

public static String sha256Hash(Object source, Object salt, int iter) {
	Sha256Hash sha256 = new Sha256Hash(source, ByteSource.Util.bytes(salt), iter);
	return sha256.toHex();
}

public static String sha384Hash(Object source, Object salt, int iter) {
	Sha384Hash sha256 = new Sha384Hash(source, ByteSource.Util.bytes(salt), iter);
	return sha256.toHex();
}

public static String sha512Hash(Object source, Object salt, int iter) {
	Sha512Hash sha512 = new Sha512Hash(source, ByteSource.Util.bytes(salt), iter);
	return sha512.toHex();
}
...
public static String simpleHash(String algorithmName, Object source, Object salt, int hashIterations) {
	SimpleHash simple = new SimpleHash(algorithmName, source, ByteSource.Util.bytes(salt), hashIterations);
	return simple.toHex();
...
private static void sysUser() {
	...
	System.out.println("salt: "+ salt+" simpleHash: "+simpleHash);
}
